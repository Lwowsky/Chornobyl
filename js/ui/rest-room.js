(function () {
  ...
  function claimDailyReward() {
    ...
    const itemMap = {
      ...
      iodine: "iodine",
      medkit: "medkit",
      rareMaterials: "uncommon_hide",
      gearCrate: "gear_crate"
    };
    FARM_FIND_KEYS.forEach((key) => {
      ...
      const itemId = itemMap[key];
      if (itemId) rewardItems.push({ id: itemId, qty: amount });
    });

    if (window.GameRewards) {
      result.inventoryReward = window.GameRewards.grant({ money: result.moneyCents / 100, items: rewardItems }, "rest-room");
    } else {
  ...
  function renderMainInfo(currentRoom, nextRoom) {
    ...
    if (view) {
      view.classList.toggle('is-owned', room.owned);
      view.classList.toggle('is-unowned', !room.owned);
      view.classList.toggle('is-rented', !room.owned && room.rented);
    }
    ...
    if (compactUpgradeCard) {
      compactUpgradeCard.hidden = !nextRoom;
    }
    ...
    if (nextRoom) {
      ...
      setText('restRoomNextTierCostLabel', room.owned ? 'Ціна покращення' : 'Ціна викупу');
      setText('restRoomNextTierCost', room.owned ? formatMoney(nextRoom.price) : formatMoney(BUY_PRICE));
      ...
      setText('restRoomCompactNextHpCurrent', room.owned ? currentRoom.hp : '—');
      setText('restRoomCompactNextHpTarget', nextRoom.hp);
      setText('restRoomCompactNextEnergyCurrent', room.owned ? currentRoom.energy : '—');
      setText('restRoomCompactNextEnergyTarget', nextRoom.energy);
      setText('restRoomCompactNextRadiationCurrent', room.owned ? currentRoom.radiation : '—');
      setText('restRoomCompactNextRadiationTarget', nextRoom.radiation);
      setText('restRoomCompactNextTierCostLabel', room.owned ? 'Ціна покращення' : 'Ціна викупу');
      setText('restRoomCompactNextTierCost', room.owned ? formatMoney(nextRoom.price) : formatMoney(BUY_PRICE));
  ...
  function primaryAction() {
    ...
    const player = window.GameState?.player;
    if (!player) return;
    if (window.GameVitals?.fullRestore) window.GameVitals.fullRestore();
    else {
